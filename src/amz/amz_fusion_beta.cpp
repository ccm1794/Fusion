...
void ImageLiDARFusion::FusionCallback()
{
  ...
  {
    std::vector<Box_yolo> boxes_2d_cluster; // 투영된 상자 좌표를 저장할 벡터 구조체
    ...
    for (const auto &Box : lidar_boxes)
    {
      ...
      if (Box.y < 0)
      {
        ...
        cv::Mat newpos13(this->transformMat_right * pos5); 
    ...
    std::vector<bool> skipped_yolo_boxes(yolo_boxes.size(), false); // 욜로상자 중복을 피하기 위한 장치
