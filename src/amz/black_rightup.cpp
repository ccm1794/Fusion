  ...
  if (this->sync_locker == 1)
  {
    ...
    if (this->yolo_locker == 0 && this->box_locker == 0)
    {
      ...
      for (const auto &Box : lidar_boxes)
      {
        ...
        if (Box.y >= 0) // 
        {
          boxes_2d_cluster.push_back(transformBox(this->transformMat_left , real_points,480));
          cv::rectangle(image, 
        ...
        if (Box.y < 0)
        {
          boxes_2d_cluster.push_back(transformBox(this->transformMat_right , real_points));
          cv::rectangle(image, 
          Rect(Point(boxes_2d_cluster.back().x1, boxes_2d_cluster.back().y1), 
            Point(boxes_2d_cluster.back().x2, boxes_2d_cluster.back().y2)), 
          Scalar(0, 0, 255), 2, 8, 0);
